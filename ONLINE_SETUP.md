...
防衛戦の防壁、固定トラップ、固定砲台、囮ビーコンはホスト端末だけが生成します。ゲストや盤面外オペレーターがボタンを押した場合は、設置要求をホストへ送り、ホストが残り資材・クールダウン・設置上限を判定します。生成された設備本体と耐久は低頻度スナップショットで全端末へ同期されます。

弧月／スコーピオンのジャスト判定と射撃の適正距離ダメージは、試合ホスト側の戦闘計算で判定されます。ガンナー／スナイパーのスコープは各参加者の端末で表示されるため、通信量は増えません。

更新後に古い画面が残る場合は、GitHub Pagesを `Ctrl + Shift + R` で強制再読み込みしてください。現行HTMLはCSSとJavaScriptを `?v=21` 付きで読み込みます。

## v20の同期

射手の準備状態、銃の残弾・リロード、近接連撃状態をホストから同期します。ダメージ距離補正、標的判断、リロード完了は試合ホストが確定します。詳細戦闘ログは `schemaVersion 20` です。HTMLはCSSとJavaScriptを `?v=21` 付きで読み込みます。
