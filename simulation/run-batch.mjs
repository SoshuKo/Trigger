...
async function buildInlineGameHtml(root) {
  ...
  const v107Css = await fs.readFile(path.join(root, 'v107-features.css'), 'utf8').catch(() => '');
  ...
  for (const source of scripts) {
    ...
    else if (source === 'js/v77-features.js' || source === 'js/v107-features.js') html = html.replace('</body>', `<script>\n${code}\n</script>\n</body>`);
  }
  return html;
}
...
async function runOne(index) {
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    try {
      ...
      result.gameVersion = Math.max(107, Number(result.gameVersion || 0));
      result.featureVersion = 107;
...
try {
  ...
  await page.waitForFunction(() => Boolean(window.TRION_SIMULATION_API?.runMatch) && Number(window.TRION_NAMED_AUDIT?.version || 0) >= 107 && Number(window.TRION_V107_AUDIT?.version || 0) >= 107, null, { timeout: 0 });
