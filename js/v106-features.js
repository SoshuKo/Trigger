(() => {
  ...
  function freePanVector() {
    let x = 0;
    let y = 0;
    if (pressedKeys.has('KeyA') || pressedKeys.has('ArrowLeft') || heldPanButtons.has('left')) x -= 1;
    if (pressedKeys.has('KeyD') || pressedKeys.has('ArrowRight') || heldPanButtons.has('right')) x += 1;
    if (pressedKeys.has('KeyW') || pressedKeys.has('ArrowUp') || heldPanButtons.has('up')) y -= 1;
    if (pressedKeys.has('KeyS') || pressedKeys.has('ArrowDown') || heldPanButtons.has('down')) y += 1;
  ...
  function mountPanel() {
    ...
    panel.id = 'v106SpectatorControls';
    panel.className = 'v106-spectator-controls hidden';
    panel.innerHTML = `
      <div class="v106-spectator-top">
        <span>CAMERA</span>
        <strong id="v106SpectatorMode">AUTO</strong>
        <b id="v106SpectatorTarget">---</b>
      </div>
      <div class="v106-spectator-modes">
        <button type="button" data-v106-mode="auto">AUTO</button>
        <button type="button" data-v106-mode="lock">LOCK</button>
        <button type="button" data-v106-mode="free">FREE</button>
      </div>
      <div class="v106-spectator-row">
        <button type="button" data-v106-target="-1">◀ PREV</button>
        <button type="button" data-v106-target="1">NEXT ▶</button>
      </div>
      <div class="v106-spectator-row v106-zoom-row">
        <button type="button" data-v106-zoom="out">−</button>
        <strong id="v106SpectatorZoom">100%</strong>
        <button type="button" data-v106-zoom="in">＋</button>
        <button type="button" data-v106-zoom="reset">RESET</button>
      </div>
      <div class="v106-pan-pad" aria-label="フリーカメラ移動">
        <button type="button" data-v106-pan="up">▲</button>
        <button type="button" data-v106-pan="left">◀</button>
        <button type="button" data-v106-pan="down">▼</button>
        <button type="button" data-v106-pan="right">▶</button>
      </div>
      <small>SPACE: モード切替 / WASD・矢印: FREE移動 / ホイール・±: ズーム</small>`;
    gameScreen.appendChild(panel);
    panelMode = panel.querySelector('#v106SpectatorMode');
    panelTarget = panel.querySelector('#v106SpectatorTarget');
    panelZoom = panel.querySelector('#v106SpectatorZoom');

    panel.addEventListener('click', (event) => {
      const button = event.target.closest('button');
      if (!button || !currentGame || !isSpectating(currentGame)) return;
      if (button.dataset.v106Mode) setSpectatorMode(currentGame, button.dataset.v106Mode);
      if (button.dataset.v106Target) cycleLockedTarget(currentGame, Number(button.dataset.v106Target));
      if (button.dataset.v106Zoom) {
        const state = gameState(currentGame);
        if (button.dataset.v106Zoom === 'in') adjustZoom(currentGame, state.zoom * 1.16);
        if (button.dataset.v106Zoom === 'out') adjustZoom(currentGame, state.zoom / 1.16);
        if (button.dataset.v106Zoom === 'reset') adjustZoom(currentGame, 1);
      }
    });

    panel.addEventListener('pointerdown', (event) => {
      const button = event.target.closest('[data-v106-pan]');
      if (!button || !currentGame || !isSpectating(currentGame)) return;
      event.preventDefault();
      setSpectatorMode(currentGame, 'free');
      heldPanButtons.add(button.dataset.v106Pan);
      button.setPointerCapture?.(event.pointerId);
    });
    panel.addEventListener('pointerup', (event) => {
      const button = event.target.closest('[data-v106-pan]');
      if (button) heldPanButtons.delete(button.dataset.v106Pan);
    });
    panel.addEventListener('pointercancel', () => heldPanButtons.clear());
    return panel;
  }
  ...
  function refreshPanel(game) {
    mountPanel();
    if (!panel) return;
    const active = isSpectating(game);
    ...
    panelTarget.textContent = state.mode === 'free' ? 'FIELD OVERVIEW' : target ? `${target.name || 'UNIT'}${target.dead ? ' / BAILED OUT' : ''}` : 'NO ACTIVE UNIT';
    panelZoom.textContent = `${Math.round(state.zoom * 100)}%`;
    panel.querySelectorAll('[data-v106-mode]').forEach((button) => button.classList.toggle('active', button.dataset.v106Mode === state.mode));
    const spectatorName = document.querySelector('#spectatorName');
    if (spectatorName) {
      if (state.mode === 'free') spectatorName.textContent = `FIELD OVERVIEW / ${Math.round(state.zoom * 100)}%`;
      else if (state.mode === 'lock' && target) spectatorName.textContent = `LOCK ${target.name || 'UNIT'}${target.dead ? ' / BAILED OUT' : ''}`;
    }
  }
  ...
  function useSupportAction(game, unit, hand, index, target, options = {}) {
    try {
      ...
      console.warn('[v106 support action]', error);
      return false;
    }
  }
  ...
  function installSimulationApi() {
    ...
    if (api.runMatch !== simulationBase && !api.runMatch.v106Wrapped) {
      ...
      wrapped.v106Wrapped = true;
      simulationBase = wrapped;
      api.runMatch = wrapped;
    }
    api.version = VERSION;
    api.v106Wrapped = true;
  }
  ...
  function bindGlobalControls() {
    ...
    window.addEventListener('blur', () => { pressedKeys.clear(); heldPanButtons.clear(); drag = null; });
    ...
    document.addEventListener('pointercancel', () => { drag = null; heldPanButtons.clear(); }, true);
  }
