(() => {
  ...
  async function usernameEmail(username) {
    ...
    if (!/^[\p{L}\p{N}_\-.]{3,20}$/u.test(normalized)) throw new Error('ユーザー名は3～20文字の文字・数字・_・-・.で入力してください。');
  ...
  service.registerAccount = async (username, password) => {
    ...
    const { data, error } = await service.client.auth.signUp({
      email: account.email,
