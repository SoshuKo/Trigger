(() => {
  ...
  function installSimulationApiV109(){const api=window.TRION_SIMULATION_API;if(!api||api.v109Wrapped||typeof api.runMatch!=='function')return;const oldRun=api.runMatch.bind(api);api.runMatch=async request=>{const result=await oldRun(request);if(result&&typeof result==='object'){result.gameVersion=Math.max(109,Number(result.gameVersion||0));result.featureVersion=109;result.namedSimulation=true;}return result;};api.version=109;api.v109Wrapped=true;}
  installSimulationApiV109();
  window.TRION_NAMED_AUDIT={version:109,agents:Object.keys(NAMED_BEHAVIORS),tunedAgents:Object.keys(NAMED_TUNING_OVERRIDES),count:Object.keys(NAMED_BEHAVIORS).length,allExplicitlyTuned:Object.keys(NAMED_BEHAVIORS).every(name=>Boolean(NAMED_TUNING_OVERRIDES[name]))};
  function syncVersionUI(){syncV96TriggerDocs();if(window.TRION_SIMULATION_API)window.TRION_SIMULATION_API.version=107;document.querySelectorAll('.version-badge,[data-version],#version,.version').forEach(el=>{if(/VERSION\s*\d+/i.test(el.textContent||'')||el.matches('.version-badge,[data-version],#version'))el.textContent='VERSION 109';});document.title=document.title.replace(/VERSION\s*\d+/ig,'VERSION 109');document.documentElement.dataset.gameVersion='109';}
  syncVersionUI();
  let lastVersionSync=Date.now();
  const timer=setInterval(()=>{const g=window.__TRION_GAME__;if(g&&g!==currentGame)patchGame(g);const now=Date.now();if(now-lastVersionSync>=1000){installSimulationApiV109();syncVersionUI();lastVersionSync=now;}document.querySelectorAll('.v77-trigger-panel').forEach(el=>el.remove());},1000);
