...
feature = Path('js/v77-features.js')
source = feature.read_text(encoding='utf-8')

new_choose = r'''  function mobilityPathOpen(g,p,angle,distance){
...
choose_pattern = re.compile(r"  function chooseOpenMobilityAngle\(g,p,base,distance=210\)\{.*?\n  \}\n(?=  function tacticalMobilityAngle)", re.S)
source, count = choose_pattern.subn(new_choose, source, count=1)
if count != 1:
    raise SystemExit(f'chooseOpenMobilityAngle replacement count: {count}')

new_recovery = r'''  function embeddedWallContact(g,p,r){
    let best=null;
...
recovery_pattern = re.compile(r"  function recoverEmbeddedPlayer\(g,p\)\{.*?\n  \}\n(?=  function updateEngineerTraps)", re.S)
source, count = recovery_pattern.subn(new_recovery, source, count=1)
if count != 1:
    raise SystemExit(f'stall recovery replacement count: {count}')
if 'open=openPointNear(g,p.x,p.y' in source:
    raise SystemExit('teleport recovery remains')
feature.write_text(source, encoding='utf-8')

Path('js/online-config.js').write_text("""/*
...
trigger = Path('js/trigger-data.js')
data = trigger.read_text(encoding='utf-8')
for old, new in {
    'damage: 49, speed: 1280': 'damage: 58, speed: 1280',
    'damage: 27, speed: 1750': 'damage: 33, speed: 1750',
    'damage: 72, speed: 820': 'damage: 84, speed: 820',
}.items():
    if old in data:
        data = data.replace(old, new, 1)
    elif new not in data:
        raise SystemExit(f'sniper source changed: {old}')
trigger.write_text(data, encoding='utf-8')
